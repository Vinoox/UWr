...
static void init() {
	...
	gameMoney = 200;
	gameLife = 1000;
	background = al_load_bitmap("mapypng/mapa3/najnowsze/mapa3_przeszkody10x10.png");
	if (!background) gameCrashLog("blad ladowania pliku mapa3");
	...
	getMap(map, 3);
	toolbar = toolbarInit();
	start = getPoints("sciezkitxt/sciezka3.txt");
