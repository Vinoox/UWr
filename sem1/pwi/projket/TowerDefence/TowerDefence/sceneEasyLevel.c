...
static void init() {
	...
	gameMoney = 200;
	gameLife = 1000;
	background = al_load_bitmap("mapypng/mapa1/najnowsze/mapa_1_przeszkody10x10.png");
	if (!background) gameCrashLog("blad ladowania pliku mapa1");
	...
	getMap(map, 1);
	toolbar = toolbarInit();
	start = getPoints("sciezkitxt/sciezka1.txt");
...
Scene sceneEasyLevelCreate()
{
	Scene scene;
	memset(&scene, 0, sizeof(Scene));
	scene.name = "Poziom pierwszy";
