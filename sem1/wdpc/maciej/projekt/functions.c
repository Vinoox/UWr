...
point* getData(char* filePath){
    ...
    while(fgets(line, sizeof(line), fp)){
        ...
        if (strncmp(line, "<trkpt ", 7) == 0){
            double lat, lon, ele = -9999;
            char time[20];
            sscanf(line, "<trkpt lat=\"%lf\" lon=\"%lf\">", &lat, &lon);

            while(strncmp(line, "</trkpt>", 8) != 0){
                fgets(line, sizeof(line), fp);
                sscanf(line, " %[^\n]", line);
                if(strncmp(line, "<ele>", 5) == 0) sscanf(line, "<ele>%lf<", &ele);
                if(strncmp(line, "<time>", 6) == 0) sscanf(line, "<time>%19[^<]</time>", time);
            }
